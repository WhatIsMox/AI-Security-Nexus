...
export const IncidentDetailModal: React.FC<IncidentDetailModalProps> = ({
  ...
  return (
    ...
    >
      <div 
        className="w-full max-w-4xl bg-slate-900 border border-slate-700/80 rounded-2xl shadow-[0_0_50px_rgba(0,0,0,0.85)] overflow-hidden flex flex-col max-h-[90vh] animate-in zoom-in-95 duration-200"
        onClick={e => e.stopPropagation()}
      >
        {/* Modal Header */}
        <div className="p-5 md:p-6 border-b border-slate-800 bg-slate-950/90 flex items-start justify-between gap-4">
          <div className="space-y-2">
            {/* Badges Bar */}
            <div className="flex flex-wrap items-center gap-2">
              <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-mono bg-amber-500/10 border border-amber-500/30 text-amber-300">
                <Flame className="w-3.5 h-3.5 text-amber-400" />
                Case Study
              </span>

              {incident.severity && (
                <span className={`inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-semibold border ${getSeverityBadge(incident.severity)}`}>
              ...
              {incident.targetOrVictim && (
                <span className="inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-mono border border-cyan-500/30 bg-cyan-500/10 text-cyan-300">
              ...
              {incident.cveOrAdvisoryId && incident.cveOrAdvisoryId !== 'N/A' && (
                <span className="inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-mono border border-purple-500/30 bg-purple-500/10 text-purple-300">
            ...
            {/* Title */}
            <h2 id="incident-modal-title" className="text-xl md:text-2xl font-bold text-white tracking-tight">
              {incident.title}
            </h2>
          </div>
          ...
          <div className="flex items-center gap-2 shrink-0">
            {incident.url && (
              <a
                ...
                className="p-2 rounded-lg bg-slate-800/80 hover:bg-cyan-500/20 text-slate-400 hover:text-cyan-300 border border-slate-700 transition-colors"
        ...
        <div className="p-5 md:p-6 overflow-y-auto space-y-6 text-sm">
          ...
          <div className="bg-slate-950/60 border border-slate-800/90 rounded-xl p-4 md:p-5 space-y-2.5">
          ...
          <div className="bg-amber-950/20 border border-amber-500/30 rounded-xl p-4 md:p-5 space-y-2">
          ...
          <div className="bg-emerald-950/20 border border-emerald-500/30 rounded-xl p-4 md:p-5 space-y-2">
          ...
          <div className="bg-slate-950/60 border border-slate-800 rounded-xl p-4 md:p-5 space-y-2">
              ...
              <div className="flex flex-wrap gap-2">
                {incident.mappedThreats.map(threatId => (
                  <button
                    ...
                    className="px-3 py-1.5 rounded-lg bg-slate-950 hover:bg-cyan-950/40 border border-slate-800 hover:border-cyan-500/40 text-cyan-400 hover:text-cyan-300 font-mono text-xs transition-colors flex items-center gap-1.5"
                  >
                    <span>{threatId}</span>
          ...
          {incident.url && (
            <div className="pt-2 border-t border-slate-800 flex items-center justify-between gap-4 flex-wrap bg-slate-950/40 p-3 rounded-lg">
              ...
              <a
                ...
                target="_blank"
                rel="noopener noreferrer"
                className="inline-flex items-center gap-1 px-3 py-1.5 rounded-md bg-amber-500/10 hover:bg-amber-500/20 text-amber-300 border border-amber-500/30 text-xs font-semibold transition-colors shrink-0"
              >
