...
function extractFrameworkStats() {
  const llmContent = readFileContent('data_llm.ts');
  const mlContent = readFileContent('data_ml.ts');
  const agenticSkillsContent = readFileContent('data_agentic.ts');
  const agenticAppsContent = readFileContent('data_agentic_applications.ts');
  const mcpContent = readFileContent('data_mcp.ts');
  const secureMcpContent = readFileContent('data_secure_mcp_guide.ts');
  const saifContent = readFileContent('data_saif.ts');
  const dsgaiContent = readFileContent('data_genai_data_security.ts');
  const testsContent = readFileContent('data_tests.ts');
  ...
  const modelTests = (testsContent.match(/pillar:\s*Pillar\.MODEL/g) || []).length +
                     (agenticSkillsContent.match(/pillar:\s*Pillar\.MODEL/g) || []).length;
  const infraTests = (testsContent.match(/pillar:\s*Pillar\.INFRA/g) || []).length +
                     (agenticSkillsContent.match(/pillar:\s*Pillar\.INFRA/g) || []).length;
  const dataTests = (testsContent.match(/pillar:\s*Pillar\.DATA/g) || []).length +
                    (agenticSkillsContent.match(/pillar:\s*Pillar\.DATA/g) || []).length;
  ...
  return {
    ...
    tests: {
      ...
      standardTestsCount: standardTestMatches.length,
      agenticTestsCount: agenticTestMatches.length,
      byPillar: {
...
function generateProjectMap(stats) {
  return `# Project Map & Architecture Topology
*Auto-generated by \`scripts/sync-agent-docs.mjs\` on ${new Date().toISOString().split('T')[0]}*
...
| :--- | :--- | :---: | :--- |
| **OWASP Top 10 for LLMs (2026)** | \`LLM01:2026\` - \`LLM10:2026\` | **${stats.frameworks.owaspLlmTop10_2026.count}** | [\`data_llm.ts\`](../data_llm.ts) |
| **OWASP ML Security Top 10** | \`ML01:2023\` - \`ML10:2023\` | **${stats.frameworks.owaspMlTop10.count}** | [\`data_ml.ts\`](../data_ml.ts) |
| **OWASP Agentic Applications Top 10** | \`ASI01\` - \`ASI10\` | **${stats.frameworks.owaspAgenticAppsTop10.count}** | [\`data_agentic_applications.ts\`](../data_agentic_applications.ts) |
| **OWASP Agentic Skills Top 10** | \`AST01\` - \`AST10\` | **${stats.frameworks.owaspAgenticSkillsTop10.count}** | [\`data_agentic.ts\`](../data_agentic.ts) |
| **OWASP MCP Top 10 (v0.1)** | \`MCP1:2025\` - \`MCP10:2025\` | **${stats.frameworks.owaspMcpTop10.count}** | [\`data_mcp.ts\`](../data_mcp.ts) |
| **Google SAIF Threat Model** | \`SAIF-R01\` - \`SAIF-R15\` | **${stats.frameworks.googleSaifThreats.count}** | [\`data_saif.ts\`](../data_saif.ts) |
| **OWASP GenAI Data Security (2026)** | \`DSGAI01\` - \`DSGAI21\` | **${stats.frameworks.owaspGenAiDataSecurity.count}** | [\`data_genai_data_security.ts\`](../data_genai_data_security.ts) |
| **AI-DSPM Posture Capabilities** | \`ai-dspm-01\` - \`ai-dspm-13\` | **${stats.frameworks.aiDspmCapabilities.count}** | [\`data_genai_data_security.ts\`](../data_genai_data_security.ts) |
| **Security Test Cases (Total)** | \`AITG-*\`, \`AGT-*\` | **${stats.tests.total}** | [\`data_tests.ts\`](../data_tests.ts) & [\`data_agentic.ts\`](../data_agentic.ts) |
| **Tooling Registry Entries** | Security Tools | **${stats.catalogs.toolsCount}** | [\`tools_catalog.ts\`](../tools_catalog.ts) |
| **Real-World Incident Citations** | CVEs / Papers / Outages | **${stats.catalogs.incidentsCount}** | [\`incidents_catalog.ts\`](../incidents_catalog.ts) |
